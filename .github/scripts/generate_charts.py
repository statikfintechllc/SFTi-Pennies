...
def generate_static_chart(trades, output_path='index.directory/assets/charts/equity-curve.png'):
    ...
    ax.plot(dates, cumulative_pnl, color='#00ff88', linewidth=2, marker='o', markersize=4)
    ...
    # Formatting
    ax.set_title('Equity Curve', fontsize=16, fontweight='bold', color='#00ff88', pad=20)
...
def generate_trade_distribution_chart(trades, output_path='index.directory/assets/charts/trade-distribution.png'):
    ...
    # Plot bars
    bars = ax.bar(trade_numbers, pnls, color=colors, alpha=0.8, edgecolor='#0a0e27', linewidth=1)
    
    # Add a zero line
    ax.axhline(y=0, color='#ffffff', linestyle='-', alpha=0.5, linewidth=1)
    
    # Formatting
    ax.set_title('Trade P&L Distribution', fontsize=16, fontweight='bold', color='#00ff88', pad=20)
