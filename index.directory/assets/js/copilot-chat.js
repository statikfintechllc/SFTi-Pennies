...
class CopilotChat {
  ...
  createCopilotButton() {
    // Find the navigation buttons group
    const navButtonsGroup = document.querySelector('.nav-buttons-group');
    if (!navButtonsGroup) {
      console.warn('Nav buttons group not found');
      return;
    }
    ...
    if (authButton) {
      navButtonsGroup.insertBefore(copilotButton, authButton);
    } else {
      navButtonsGroup.appendChild(copilotButton);
    }
  }
